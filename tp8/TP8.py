...
def shape_selection3(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ref_point.append((x, y))
        cv2.circle(image3, ref_point[i], 2, (0, 255, 0), 2)
...
def transformation3(himagen):

    p1 = np.float32([[0, 0],
                     [0, 500],
                     [1000, 500],
                     [1000, 0]])
    p2 = np.float32([[ref_point[0]],
                     [ref_point[3]],
                     [ref_point[2]],
                     [ref_point[1]]])
    m1 = cv2.getPerspectiveTransform(p2, p1)
    himagen = cv2.warpPerspective(himagen, m1, (1000, 500))
...
while 1:
    ...
    if key == ord("r"):
        ...
        cv2.setMouseCallback("image3", shape_selection3)
        ...
        resultado2 = transformation3(clone2)
        cv2.imshow("image3", resultado2)
        cv2.imwrite('lenna6.jpg', resultado2)
        break
    elif key == ord("q"):
        break
